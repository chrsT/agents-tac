...
private void flightBids() {
  ...
  private float predicted_increase_multiplier = 1.2f; //TODO: Take into account the fact that one hotel auction is closed
  ...
  private int predictCost(int day_in, int day_out, int hotel_type)
  {
	  ...
	  if (items_available[inflight_auction] > 0)
	  {
		COST_Inflight = 0;
		  ...
		  COST_Inflight = agent.getQuote(inflight_auction).getBidPrice();
	  }
	  ...
	  if (items_available[outflight_auction] > 0)
	  {
		COST_Outflight = 0;
		  ...
		  COST_Outflight = P_Cost += agent.getQuote(outflight_auction).getBidPrice();
	  }
	  ...
	  for (int i = day_in; i < day_out; i++)
	  {
		  ...
		  COST_Hotel += (agent.getQuote(agent.getAuctionFor(TACAgent.CAT_HOTEL, hotel_type, i)).getBidPrice() + predicted_increase_static) * predicted_increase;
	  }
